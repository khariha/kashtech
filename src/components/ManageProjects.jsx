...
const ManageProjects = ({ companyId, companyName, onClose }) => {
    ...
    const handleDelete = async (sow_id) => {
        ...
        try {
            const token = localStorage.getItem("token");
            await axios.delete(API.FETCH_PROJECT_DETAILS(sowId), {
                headers: { Authorization: `Bearer ${token}` },
            });
            fetchProjects();
        } catch (err) {
                ...
                <div className="grid grid-cols-2 gap-8">
                    <div>
                        <div className="flex justify-between items-center mb-2">
                            ...
                            <button onClick={() => setEditingProject(null)} className="text-purple-600 font-medium">+ Add Project</button>
                        </div>
                            ...
                            {projects.map((proj, idx) => (
                                <li key={proj.sow_id} className="flex justify-between items-center mb-2 bg-white p-2 rounded shadow">
                                    ...
                                    <div className="flex gap-2">
                                        <button onClick={() => handleEdit(proj)}><FaEdit className="text-purple-600" /></button>
                                        <button onClick={() => handleDelete(proj.sow_id)}><FaTrash className="text-red-600" /></button>
                                    </div>
                                </li>
                            ))}
                        </ul>
                    ...
                    <div>
                        ...
                        <div className="mb-2">
                            ...
                            <div className="flex gap-2 mb-2">
                                <select value={selectedEmployee} onChange={(e) => setSelectedEmployee(e.target.value)} className="flex-1 border rounded px-3 py-2">
                                    <option value="">Select Employee</option>
                                    {employees.map((emp) => (
                                        <option key={emp.kash_operations_usn} value={emp.kash_operations_usn}>{emp.first_name} {emp.last_name}</option>
                                    ))}
                                </select>
                                <button onClick={handleAddEmployee} className="bg-purple-600 text-white px-4 rounded">+ Add</button>
                            </div>
                            <div className="flex flex-wrap gap-2">
                                {formData.assigned_employees.map((emp) => (
                                    <span key={emp} className="bg-purple-100 text-purple-800 px-2 py-1 rounded-full text-sm flex items-center">
                                        {emp}
                                        <button onClick={() => handleRemoveEmployee(emp)} className="ml-2 text-red-500">×</button>
                                    </span>
                                ))}
                            </div>
                        </div>
